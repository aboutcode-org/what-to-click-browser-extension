...
function StepDescription(child) {
  return p(
    ...
    span({ class: "text-content" }, span({ class: "index" }), child),
    button(
      { class: "text-button delete-button", "wtc-editor": 1 },
      "Remove step",
    ),
  );
}
...
export function StartingStep({ url }) {
  return div(
    { class: "step", "wtc-step-index": 1 },
    StepDescription(
      span({ class: "content" }, ["Visit ", a({ href: url }, url), "."]),
    ),
  );
}
...
export function BackNavigationStep({ url, index }) {
  return div(
    { class: "step", "wtc-step-index": index },
    StepDescription(
      span({ class: "content" }, ["Go back to ", a({ href: url }, url), "."]),
    ),
  );
}

export function ScreenshotStep({ image, target }, index) {
  const actionDescription = tagToName[target.tagName]
    ? `${tagToName[target.tagName]}`
    : "";

  return div(
    { class: "step", "wtc-step-index": index + 2 },
    StepDescription(
      textarea(
        { class: "content", "wtc-textarea": 0 },
