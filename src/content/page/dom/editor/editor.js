  ...
  for (const screenshot of screenshots) {
    const observer = new MutationObserver((mutations, observer) => {
      ...
      for (const { word, box } of words) {
        const width =
          (((box.x1 - box.x0) * sizeRatio) / clientSize.width) * 100;
        const height =
          (((box.y1 - box.y0) * sizeRatio) / clientSize.height) * 100;
        ...
        const scrubElementHtml = `
          ...
          ></div>
        `;
        const scrubElement = parser
          .parseFromString(scrubElementHtml, "text/html")
          .querySelector(".scrub-element");
