...
export async function saveHtml() {
  ...
  const documentToExport = new DOMParser().parseFromString(
    pageHtml,
    "text/html",
  );
  documentToExport
  ...
  documentToExport
    .querySelectorAll(".scrub-overlay")
    .forEach((element) => element.remove());
  documentToExport
    .querySelectorAll("[wtc-ocr]")
    .forEach((element) => element.removeAttribute("wtc-ocr"));
  documentToExport
  ...
  const htmlContent = documentToExport
    .querySelector("html")
    .innerHTML.replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">");
  const page = `
  ...
  downloadURI(
    `data:text/html,${page}`,
    `What to click ${new Date().toDateString()}.html`,
  );
  await removeScrubs();
}
