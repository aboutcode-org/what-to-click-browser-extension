...
export function applyScrubs(screenshot) {
  ...
  const scrubs = screenshot.parentNode.querySelectorAll(
    ".scrub-overlay>.scrubbed",
  );
  for (const scrub of scrubs) {
    const { box } = JSON.parse(
      decodeURIComponent(scrub.getAttribute("wtc-word")),
    );
...
export async function removeScrubs() {
  ...
  screenshots.forEach((screenshot) => {
    const stepIndex =
      parseInt(
        screenshot.parentElement.parentElement.parentElement.getAttribute(
          "wtc-step-index",
        ),
        10,
      ) - 2;
