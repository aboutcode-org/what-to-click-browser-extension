browser.runtime.onMessage.addListener(
  async ({ type = "general", data = {} }, sender) => {
    if (type === "mousedown") {
      ...
      await localforage.setItem(sessionKey, [
  ...
  if (sessionActive) {
    ...
    if ((await localforage.getItem(`images-${sessionActive}`)).length > 0) {
      await browser.tabs.create({
        url: `/content/page.html?s=${encodeURIComponent(sessionActive)}`,
        active: false,
      });
    }
  } else {
    ...
    await localforage.setItem("sessions", [
      ...((await localforage.getItem("sessions")) || []),
      session,
    ]);
